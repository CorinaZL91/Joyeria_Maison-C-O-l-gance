...
import { useGetCategoryByIdQuery } from "@/app/state/services/categoryApi";
...
export default function CategoryProductsClient() {
  ...
  const {
    ...
    skip: Number.isNaN(categoryId) || categoryId <= 0,
  });
  ...
  const filteredProducts = useMemo(() => {
    const filtered = visibleProducts.filter((product) => {
      ...
      return matchesMin && matchesMax && matchesMaterial;
    });
    ...
    return filtered;
  }, [visibleProducts, minPrice, maxPrice, sortBy, selectedMaterial]);
  ...
  if (isLoadingCategory || (matchedCategory && isLoadingProducts)) {
    return (
      <main className="min-h-screen bg-background px-4 pb-16 pt-36 md:px-8 md:pt-40">
        <div className="mx-auto max-w-7xl rounded-[28px] border border-border bg-card py-16 text-center text-foreground/60">
          Cargando productos...
  ...
  if (isCategoryError || !matchedCategory) {
    return (
      <main className="min-h-screen bg-background px-4 pb-16 pt-36 md:px-8 md:pt-40">
        <div className="mx-auto max-w-7xl rounded-[28px] border border-border bg-card py-16 text-center text-foreground/60">
        ...
        <section className="relative overflow-hidden rounded-[36px] border border-[#ead4da] bg-gradient-to-br from-[#f8e7eb] via-[#f7ecef] to-[#fcf7f8] px-6 py-14 text-center shadow-sm dark:border-border dark:bg-card md:px-12 md:py-16">
          <div className="mx-auto max-w-3xl">
            ...
            <p className="mx-auto mt-5 max-w-2xl text-sm leading-7 text-[#9b7883] dark:text-foreground/70 md:text-base">
              Descubre nuestros productos de{" "}
              {matchedCategory.nombre.toLowerCase()}, con diseños elegantes y
              detalles encantadores para cada estilo.
            </p>
          </div>
        </section>
        ...
        <section className="mt-12">
          <div className="mb-10">
            ...
            <p className="mt-2 max-w-2xl text-sm text-foreground/65">
              Explora todos los productos disponibles en esta categoría y usa
              los filtros para encontrar la pieza ideal.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 gap-8 lg:grid-cols-[270px_minmax(0,1fr)]">
            ...
            <div>
              ...
              {!isProductsError && filteredProducts.length === 0 && (
